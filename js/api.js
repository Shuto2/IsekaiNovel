...
async function generateBackgroundImage(situationText) {
  const prompt = `
あなたはプロの漫画家です。以下の状況説明テキストを読んで、そのシーンに合う背景の線画をSVG形式で生成してください。

# 依頼内容
- 状況に合った背景を、白黒の線画で、漫画の1コマのように描いてください。
- スタイルは日本の漫画やライトノベルの挿絵のような、繊細で雰囲気のあるタッチでお願いします。
...
`;
    ...
    // --- 背景生成ロジック ---
    // これがエピソードの最初のターンである場合、背景画像を生成する
    const episodeIndex = project.episodes.findIndex(e => e.id === episode.id);
    const isFirstTurn = episode.turns.length === 1 && episode.turns[0].id === turn.id;
    if (episodeIndex === 0 && isFirstTurn && aiOutput.narration) {
